...
function renderAssignmentsTable() {
    ...
    const reqTipoNorm = (reqTipoPresupuesto || 'Eléctrico').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const activeRubro = reqTipoNorm.includes('mec') ? 'Mecánico' : 'Eléctrico';

    // Filtrar pedidos por rubro activo (Eléctrico vs Mecánico) de manera 100% estricta
    let filtered = appData.pedidos.filter(p => {
        ...
        const pRubro = isMec ? 'Mecánico' : 'Eléctrico';

        // FILTRO ESTRICTO: Solo mostrar presupuestos del rubro seleccionado
        if (pRubro !== activeRubro) {
            return false;
        }
    ...
    if (filtered.length === 0) {
        listBody.innerHTML = `<tr><td colspan="${totalCols}" style="text-align: center; color: var(--text-muted); padding: 30px; font-size: 13px;">No se encontraron presupuestos del rubro <strong>${activeRubro === 'Mecánico' ? '⚙️ Mecánico (101-MEC)' : '⚡ Eléctrico (102-ELEC)'}</strong> para los filtros seleccionados.</td></tr>`;
        return;
    }
    ...
    function createBudgetTableRow(p) {
        ...
        const tipoPresBadge = isItemMec 
            ? `<span class="badge" style="background: rgba(6, 182, 212, 0.2); color: #22d3ee; font-size: 10px; padding: 2px 7px; border: 1px solid rgba(6, 182, 212, 0.4); margin-left: 6px; border-radius: 4px; font-weight: 800;">⚙️ Mecánico</span>`
            : `<span class="badge" style="background: rgba(234, 179, 8, 0.2); color: #fde047; font-size: 10px; padding: 2px 7px; border: 1px solid rgba(234, 179, 8, 0.4); margin-left: 6px; border-radius: 4px; font-weight: 800;">⚡ Eléctrico</span>`;
        ...
        let cellsHtml = activeCols.map(colKey => {
            switch (colKey) {
                ...
                case 'cliente':
                    ...
                    return `
                        <td>
                            <div class="client-name-cell" style="line-height: 1.3;">
                                <span class="client-name" style="font-size: 12px; font-weight: 700; color: white;">${p.meca_denominacion || p.motivo || 'Cotización de Servicio'} ${tipoPresBadge}</span>
                                <span class="client-id" style="font-size: 10.5px; color: var(--text-muted); display: block; margin-top: 2px;">Cliente: <strong style="color: #cbd5e1;">${p.cliente_nombre || 'Cliente'}</strong> ${p.meca_planta ? '| Planta: ' + p.meca_planta : ''} ${p.meca_nro_oc || p.nro_oc ? '| OC: ' + (p.meca_nro_oc || p.nro_oc) : ''}</span>
                                ${avanceBadge}
                                ${alertFaltaFacturar}
                            </div>
                        </td>`;
...
window.verDetallePedido = function(id, explicitMode) {
    ...
    const isEditingAllowed = isEditRequested && segPerms.canEdit && pedido.estado !== 'Rechazado';
    ...
    if (mecaHeaderBox) {
        ...
        if (canEditControls) {
            setElemHtml('auth-meca-denominacion-val', `<input type="text" id="auth-edit-meca-denominacion" value="${devVal}" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px; font-weight: bold;">`);
            setElemHtml('auth-meca-cliente-val', `<input type="text" id="auth-edit-meca-cliente" value="${p.cliente_nombre || 'CARGILL SACI'}" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px; font-weight: bold;">`);
            setElemHtml('auth-meca-proveedor-val', `<input type="text" id="auth-edit-meca-proveedor" value="${p.meca_proveedor || 'SG MONTAJES SRL'}" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px;">`);
            setElemHtml('auth-meca-oferta-val', `<input type="date" id="auth-edit-meca-oferta" value="${p.meca_fecha_oferta || p.fecha || ''}" oninput="saveTempEdits()" style="width: 130px; font-size: 11px; padding: 3px 6px; color: #fde047; background: rgba(15, 23, 42, 0.85); border: 1px solid #eab308; border-radius: 4px; font-weight: bold;">`);
            setElemHtml('auth-meca-validez-val', `<input type="text" id="auth-edit-meca-validez" value="${p.meca_validez || '5 DÍAS'}" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px;">`);
            setElemHtml('auth-meca-planta-val', `
                <select id="auth-edit-meca-planta" onchange="saveTempEdits()" style="width: 120px; font-size: 11px; padding: 3px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px; font-weight: bold;">
                    <option value="PGSM" ${p.meca_planta === 'PGSM' ? 'selected' : ''}>PGSM</option>
                    <option value="VGG" ${p.meca_planta !== 'PGSM' ? 'selected' : ''}>VGG</option>
                </select>
            `);
            setElemHtml('auth-meca-inicio-val', `<input type="date" id="auth-edit-meca-inicio" value="${p.meca_fecha_inicio || ''}" oninput="saveTempEdits()" style="width: 130px; font-size: 11px; padding: 3px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px; font-weight: bold;">`);
            setElemHtml('auth-meca-duracion-val', `<input type="text" id="auth-edit-meca-duracion" value="${p.meca_duracion || ''}" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px;">`);
            setElemHtml('auth-meca-fin-val', `<input type="date" id="auth-edit-meca-fin" value="${p.meca_fecha_fin || ''}" oninput="saveTempEdits()" style="width: 130px; font-size: 11px; padding: 3px 6px; color: #fde047; background: rgba(15, 23, 42, 0.85); border: 1px solid #eab308; border-radius: 4px; font-weight: bold;">`);
            setElemHtml('auth-meca-propuesta-val', `<textarea id="auth-edit-meca-propuesta" rows="2" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px;">${p.meca_propuesta || ''}</textarea>`);
            setElemHtml('auth-meca-personal-val', `<input type="text" id="auth-edit-meca-personal" value="${p.meca_personal || ''}" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px;">`);
            setElemHtml('auth-meca-exclusiones-val', `<textarea id="auth-edit-meca-exclusiones" rows="2" oninput="saveTempEdits()" style="width: 100%; font-size: 11px; padding: 4px 6px; color: white; background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 4px;">${p.meca_exclusiones || ''}</textarea>`);
        } else {
    ...
    if (legacyBottom) {
        legacyBottom.style.display = 'none';
    }
    ...
    function renderModalReportTable(p, currentMode) {
        ...
        const isEditMode = canEditControls && (currentMode === 'editar' || isEditingAllowed);
        ...
        const obsHtml = (p.motivo && p.motivo.trim() && p.motivo.trim().toLowerCase() !== 'sin observaciones')
            ? `<div style="margin-top: 10px; padding: 10px 14px; background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 8px; font-size: 11.5px; text-align: left;">
                <strong style="color: #38bdf8;"><i class="fas fa-comment-alt"></i> Observaciones:</strong> <span style="color: #ffffff; margin-left: 6px;">${p.motivo.trim()}</span>
               </div>`
            : '';
        ...
        if (isRejectedOrder) {
            ...
            // ================= COMPROBANTE RESUMIDO (SIEMPRE SOLO LECTURA, NUNCA EDITABLE) =================
            ...
            // ================= COMPROBANTE DETALLADO (SOLO CONSULTA) =================
    ...
    window.cambiarTipoReporteEnVista = function(nuevoTipo) {
        ...
        if (pedidoActivo) {
            ...
            renderModalReportTable(pedidoActivo, isEditingAllowed ? 'editar' : 'ver');
        }
    };
